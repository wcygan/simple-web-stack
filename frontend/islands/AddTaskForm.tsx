    ...
    <form onSubmit={handleSubmit} class="flex flex-col sm:flex-row gap-2 mb-4">
      <input
        ...
        onInput={(e) => {
          ...
          if (error.value) error.value = null; // Clear error on input
        }}
        placeholder="Enter new task title..."
        class="flex-grow p-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500"
        disabled={isLoading.value}
      />
      <Button type="submit" disabled={isLoading.value || !title.value.trim()} class="w-full sm:w-auto">
        {isLoading.value ? "Adding..." : "Add Task"}
      </Button>
      {error.value && <p class="text-red-500 text-sm mt-1 w-full">{error.value}</p>}
    </form>
  );
} 