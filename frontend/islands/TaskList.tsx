...
const initialTasks: Task[] = [
  { id: "1", title: "Learn Deno Fresh", completed: true, created_at: new Date().toISOString(), updated_at: new Date().toISOString() },
  { id: "2", title: "Build a task app", completed: false, created_at: new Date().toISOString(), updated_at: new Date().toISOString() },
  { id: "3", title: "Deploy to the cloud", completed: false, created_at: new Date().toISOString(), updated_at: new Date().toISOString() },
];
  ...
  return (
    <ul class="list-none pl-0 mt-6 space-y-2">
      {tasksSignal.value.map((task) => (
        <li key={task.id} class="flex items-center justify-between p-3 bg-white border border-gray-200 rounded-lg shadow-sm">
          <span
            class={`cursor-pointer ${task.completed ? "line-through text-gray-500" : "text-gray-800"}`}
